...
const statusColors = (status: string, C: ColorScheme) => {
  switch (status) {
    ...
    case 'expired':
      return { bg: C.surface, text: C.textTertiary };
    default:
...
const SubCard = ({
  ...
  const totalDays = Math.max(
    1,
    Math.ceil((new Date(sub.endDate).getTime() - new Date(sub.startDate).getTime()) / 86400000),
  );
        ...
        {subs.length > 0 && (
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <View style={[S.statBadge, { backgroundColor: C.secondaryMuted }]}>
              <Text style={[S.statNum, { color: C.secondary }]}>
