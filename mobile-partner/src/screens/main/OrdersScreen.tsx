...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; nextAction?: string; nextStatus?: string }> = {
  scheduled:       { label: 'Scheduled',       color: '#3B82F6', bg: '#1E3A5F', nextAction: 'Start Preparing', nextStatus: 'preparing' },
  preparing:       { label: 'Preparing',        color: '#F59E0B', bg: '#451A03', nextAction: 'Out for Delivery', nextStatus: 'out_for_delivery' },
  out_for_delivery:{ label: 'Out for Delivery', color: '#8B5CF6', bg: '#2E1065', nextAction: 'Mark Delivered', nextStatus: 'delivered' },
  delivered:       { label: 'Delivered',        color: '#10B981', bg: '#064E3B', nextAction: undefined, nextStatus: undefined },
  failed:          { label: 'Failed',           color: '#EF4444', bg: '#450A0A', nextAction: undefined, nextStatus: undefined },
};
...
const OrdersScreen = () => {
  ...
  const updateStatus = async (deliveryId: string, newStatus: string) => {
    ...
    try {
      ...
      setDeliveries((prev) =>
        prev.map((d) => d._id === deliveryId ? { ...d, status: newStatus as Delivery['status'] } : d)
      );
    } catch (err: any) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.pageHeader}>
        ...
        <Text style={styles.pageSubtitle}>{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchDeliveries(); }} tintColor="#F59E0B" />}
          ListEmptyComponent={
