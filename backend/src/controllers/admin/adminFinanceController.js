  ...
  try {
    ...
    const [payments, total, summary] = await Promise.all([
      Payment.find(query)
        ...
        .skip((page - 1) * limit)
        .limit(limit)
        .lean(), // ⚡ Bolt: Use .lean() for read-only queries to skip Mongoose document hydration
...
exports.getPendingPayouts = async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: pendingPayouts,
      summary: {
        totalPendingAmount: pendingPayouts.reduce((sum, p) => sum + p.pendingAmount, 0),
...
exports.getPayoutHistory = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: payouts,
      summary: { totalPayouts: total, totalAmount: totalAmount[0]?.total || 0 },
      pagination: { page, limit, total, pages: Math.ceil(total / limit) },
    });
  } catch (error) {
...
exports.adminProcessRefund = async (req, res) => {
  try {
    ...
    emitNotification(payment.user, {
      ...
      message: `Your payment of ₹${refundAmount} has been refunded. Reason: ${reason || 'N/A'}`,
      type: 'info',
    });
    res.json({
      success: true,
      message: 'Refund processed successfully',
      data: { paymentId: payment._id, refundAmount, reason },
    });
  } catch (error) {
...
exports.resolveDispute = async (req, res) => {
  try {
    ...
    if (!payment.isDisputed)
      return res.status(400).json({ success: false, message: 'This payment is not disputed' });
    ...
    res.json({
      ...
      data: { paymentId: payment._id, resolution, action },
    });
  } catch (error) {
