...
exports.getCustomerAnalytics = async (req, res) => {
  try {
    ...
    const [
      ...
      // Previous period Average Lifetime Value
      Payment.aggregate([
        { $match: { status: 'paid', createdAt: { $lt: startDate } } },
        { $group: { _id: '$user', totalSpent: { $sum: '$amount' } } },
        { $group: { _id: null, avgLifetimeValue: { $avg: '$totalSpent' } } },
      ]),
      Subscription.countDocuments({ status: 'active', createdAt: { $lt: startDate } }),
      Subscription.countDocuments({ status: 'cancelled', createdAt: { $lt: startDate } }),
    ]);

    // Derived calculations
    const churnRate =
      activeSubscriptions + inactiveSubscriptions > 0
        ? ((inactiveSubscriptions / (activeSubscriptions + inactiveSubscriptions)) * 100).toFixed(2)
        : 0;

    const previousChurnRate =
      retentionData[1] + retentionData[2] > 0 // Using the new indices
        ? ((retentionData[2] / (retentionData[1] + retentionData[2])) * 100).toFixed(2)
        : 0;

    const churnRateGrowth =
      previousChurnRate > 0
        ? (((churnRate - previousChurnRate) / previousChurnRate) * 100).toFixed(1)
        : 0;
    ...
    // Calculate previous new customers for newCustomersGrowth
    // previousCustomers is new customers from the previous period!
    const newCustomersGrowth =
      previousCustomers > 0
        ? (((newCustomers - previousCustomers) / previousCustomers) * 100).toFixed(1)
        : 0;

    const currentLTV = lifetimeValue[0]?.avgLifetimeValue || 0;
    const previousLTV = retentionData[0]?.[0]?.avgLifetimeValue || 0; // The aggregate returns an array

    const avgLifetimeValueGrowth =
      previousLTV > 0 ? (((currentLTV - previousLTV) / previousLTV) * 100).toFixed(1) : 0;
    ...
    res.json({
      ...
      data: {
        overview: {
          ...
          avgLifetimeValue: Math.round(currentLTV),
          customerGrowth: parseFloat(customerGrowth),
          newCustomersGrowth: parseFloat(newCustomersGrowth),
          avgLifetimeValueGrowth: parseFloat(avgLifetimeValueGrowth),
          churnRateGrowth: parseFloat(churnRateGrowth),
        },
        acquisition,
        retention,
        topCustomers,
