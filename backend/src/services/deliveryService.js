...
const generateDeliveriesForSubscription = async (subscription) => {
    try {
        ...
        if (!tiffin || !partner) {
            logger.error(`generateDeliveries: missing populated tiffin or partner for sub ${subscription._id}`);
        ...
        if (deliveriesToCreate.length > 0) {
            ...
            logger.info(`Generated ${deliveriesToCreate.length} deliveries for subscription ${subscription._id}`);
        } else {
            logger.warn(`No deliveries generated for subscription ${subscription._id}. Check working days/tiffin days.`);
        }
