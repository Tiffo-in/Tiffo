...
## 2026-07-28 - Mongoose Read-only Query Optimization\n**Learning:** Found Mongoose `.find()` queries in `adminFinanceController.js` that were fetching arrays of documents exclusively for sending them in JSON responses, without modifying or saving them. Without `.lean()`, Mongoose hydrates these documents, consuming unnecessary CPU time and memory.\n**Action:** Always add `.lean()` to Mongoose `.find()` and `.findOne()` queries when the retrieved documents are strictly read-only and won't be saved back to the database. This significantly reduces the overhead.
