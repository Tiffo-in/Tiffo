...
## 2026-07-03 - Optimize read-only Delivery.find queries with .lean()
**Learning:** For read-only Mongoose queries like generating reports or calendars, the raw MongoDB result is sufficient and often better. Standard Mongoose queries hydrate results into full Document instances containing virtuals, change tracking, and document methods like `.save()`, which takes more CPU time and memory. Using `.lean()` skips this hydration, converting the output to plain JavaScript objects, thus freeing resources and improving processing speed. Note that `.lean()` shouldn't be used when updating documents via `.save()` or when Mongoose specific methods are required. Test mock return values should accurately reflect query chains, e.g., if `.lean()` is used, tests must mock its method returning `this`.
**Action:** When working on large datasets for aggregation, analytics or presentation use `.lean()`, provided Mongoose specific document behaviors aren't required. Always update tests respectively.
