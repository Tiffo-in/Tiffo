...
const AdminAlerts = () => {
    ...
    const loadAlertData = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                setAlerts(res.data.data.alerts.map(alert => ({
                    ...alert,
    ...
    const getAlertIcon = (type) => {
        switch (type) {
            case 'critical': return <XCircleIcon className="w-5 h-5 text-white" />;
            case 'warning': return <ExclamationTriangleIcon className="w-5 h-5 text-white" />;
            case 'info': return <InformationCircleIcon className="w-5 h-5 text-white" />;
            default: return <BellAlertIcon className="w-5 h-5 text-white" />;
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                        ...
                        >
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <span className={`w-2 h-2 rounded-full ${item.status === 'healthy' ? 'bg-green-500' : item.status === 'warning' ? 'bg-amber-500' : 'bg-red-500'}`} />
                ...
                >
                    {[
                        ...
                        >
                            {tab.label}
                            <span className={`px-1.5 py-0.5 rounded-full text-xs ${filter === tab.key ? 'bg-white/20' : 'bg-neutral-100'
                                }`}>
                                {tab.count}
                            </span>
                        </button>
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${styles.badge}`}>
                                                    {alert.type}
                                                </span>
                                                {alert.acknowledged && (
