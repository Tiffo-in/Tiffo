...
const AdminFraud = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'open':
        return <ExclamationTriangleIcon className="w-4 h-4 mr-1" />;
      case 'under_investigation':
        return <MagnifyingGlassIcon className="w-4 h-4 mr-1" />;
      case 'action_taken':
        return <CheckBadgeIcon className="w-4 h-4 mr-1" />;
      case 'dismissed':
        return <NoSymbolIcon className="w-4 h-4 mr-1" />;
      default:
        return <ShieldExclamationIcon className="w-4 h-4 mr-1" />;
    }
  };
                  ...
                  {reports.map((report, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        {report.partnerName && (
                          <div className="text-sm text-gray-700">Partner: {report.partnerName}</div>
                        )}
                        {report.orderId && (
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`flex items-center justify-center px-3 py-1 rounded-full border text-xs font-semibold capitalize whitespace-nowrap ${getStatusStyle(report.status)}`}
                        >
