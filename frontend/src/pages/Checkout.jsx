            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    try {
                        ...
                        toast.error('Payment could not be verified. If money was deducted, it will be refunded automatically.');
                        navigate(`/payment/failed?subscription=${subscriptionId}`);
                    }
                },
                prefill: {
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="ml-4 flex-1">
                                    <h3 className="font-semibold text-gray-900">{subscription.tiffin?.title || 'Tiffin Service'}</h3>
                                    <p className="text-sm text-gray-600">by {subscription.partner?.businessName || subscription.partner?.name || 'Partner'}</p>
                            ...
                            <div className="border-t pt-4 space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">{new Date(subscription.startDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                ...
                                {hasSavings && (
                                    <>
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="text-gray-400 line-through">₹{subscription.originalAmount.toLocaleString('en-IN')}</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="text-green-700 font-semibold flex items-center gap-1">
                                                🏷️ {subscription.discountLabel || `${subscription.discountPercent}% Discount`}
                                            </span>
                                        ...
                                        <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 flex items-center justify-between text-sm">
                                            ...
                                            <span className="text-green-700 font-bold">₹{(subscription.originalAmount - subtotal).toLocaleString('en-IN')}</span>
                                        </div>
                                    </>
                                )}
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                        <p className="text-sm text-blue-800">
                            🔒 <strong>Secure Payment:</strong> Your payment is processed securely through Razorpay. We never store your card details.
                        </p>
                    </div>
                    ...
                    <div className="text-center mt-4">
                        <button onClick={() => navigate(-1)} className="text-gray-500 hover:text-gray-800 text-sm transition-colors">
                            ← Cancel and go back
                        </button>
                    </div>
