  ...
  const loadDeliveryData = async () => {
    ...
    try {
      ...
      if (overviewRes.data.success && listRes.data.success) {
        ...
        const totalToday = Object.values(todayData).reduce(
        ...
        setDeliveries(
          listRes.data.data.map((del) => ({
            ...
            status: del.status,
            scheduledTime: del.deliveryDate
              ? new Date(del.deliveryDate).toLocaleTimeString('en-US', {
                  hour: '2-digit',
                  minute: '2-digit',
                })
              : 'N/A',
            deliveredTime: del.deliveredAt
              ? new Date(del.deliveredAt).toLocaleTimeString('en-US', {
                  hour: '2-digit',
                  minute: '2-digit',
                })
              : null,
            meal:
              del.subscription?.plan?.charAt(0).toUpperCase() + del.subscription?.plan?.slice(1) ||
              'Standard',
        ...
        >
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
              ...
              {['all', 'delivered', 'in_transit', 'preparing', 'pending', 'failed'].map(
                (status) => (
                  <button
                    key={status}
              ...
              <tbody className="divide-y divide-neutral-100">
                {filteredDeliveries.map((del) => (
                  <tr key={del.id} className="hover:bg-neutral-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(del.status)}`}
                      >
