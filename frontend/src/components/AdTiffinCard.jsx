...
const AdTiffinCard = ({ campaign, observeRef }) => {
  ...
  const formattedTiffin = campaign.tiffin || {
    ...
    title: campaign.menuOfTheDay
      ? `Special: ${campaign.menuOfTheDay}`
      : `${campaign.partner.businessName} Signature`,
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-3 -right-3 z-20 flex items-center bg-gradient-to-r from-orange-400 to-amber-500 text-neutral-900 text-xs font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-card border-2 border-white">
        <SparklesIcon className="w-3 h-3 text-yellow-100 mr-1" /> Sponsored
      </div>

      {campaign.hasTrialMealBoost && (
        <div className="absolute -bottom-3 -left-3 z-20 flex items-center bg-gradient-to-r from-fuchsia-600 to-rose-500 text-neutral-900 text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-card border-2 border-white">
          🔥 Trial Meal ₹{campaign.trialMealPrice}
        </div>
      )}

      {/* Inner Tiffin Card - Rendered exactly as standard */}
      <div className="relative z-10 bg-white rounded-2xl h-full border border-amber-200 overflow-hidden">
