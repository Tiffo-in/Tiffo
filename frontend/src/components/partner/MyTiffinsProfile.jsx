      ...
      <form onSubmit={handleProfileUpdate} className="p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" value={profile.businessName}
              ...
              className="input-field" placeholder="Enter your business name" disabled={loading} />
          </div>
          <div>
            <label className="block text-sm font-semibold text-neutral-700 mb-2">Phone Number</label>
            <input type="tel" value={profile.contact.phone}
              onChange={e => setProfile({ ...profile, contact: { ...profile.contact, phone: e.target.value } })}
              className="input-field" placeholder="+91 98765 43210" disabled={loading} />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={profile.description}
            ...
            placeholder="Describe your tiffin service..." disabled={loading} />
        </div>
        ...
        <div>
          ...
          <input type="text" value={profile.address.street}
            ...
            className="input-field" placeholder="Street address" disabled={loading} />
        </div>

        <div className="pt-4 border-t border-neutral-100">
          <label className="block text-sm font-semibold text-neutral-700 mb-2">Food Gallery</label>
          <p className="text-sm text-neutral-500 mb-4">Upload images of the tiffins you serve to attract more customers.</p>
          <div className="flex flex-wrap gap-4 mb-4">
            {profile.foodImages?.map((imgUrl, idx) => (
              <div key={idx} className="relative w-24 h-24 rounded-xl overflow-hidden shadow-sm border border-neutral-200">
                ...
                <button type="button"
                  onClick={() => setProfile({ ...profile, foodImages: profile.foodImages.filter((_, i) => i !== idx) })}
            ...
            {(!profile.foodImages || profile.foodImages.length < 5) && (
              <ImageUpload label="" value=""
                onChange={url => setProfile({ ...profile, foodImages: [...profile.foodImages, url] })}
                context="tiffin" />
            )}
          </div>
        </div>
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          type="submit" disabled={loading} className="btn-primary w-full md:w-auto">
